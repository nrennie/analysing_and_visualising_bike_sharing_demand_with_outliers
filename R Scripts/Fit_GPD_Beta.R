...
#plot bookings
#which(names(start_station_matrix) == "31075")
...
df <- data.frame(exceedances)

p0 <- ggplot(df, aes(x=exceedances)) + 
  geom_histogram(aes(y=..density..), binwidth=0.04, colour="black", fill="lightgray", position="identity", center=0.02) +
  geom_density(color="purple4", fill="purple4", alpha=0.4, kernel="g") + 
  xlab(expression('z'[n])) + ylab("Density") + xlim(-1,1) + ylim(0,14) +
  theme_light() + 
  labs(caption="(a) Empirical distribution") +
...
c <- apply(x, 1 ,function(y) sum(y[y>0]))
exceedances <- na.omit(c[c > 0.02])
#as.numeric(exceedances)
m <- fitgpd(data=c, threshold=0,  est = "mle")
df <- data.frame(exceedances)
...
p1 <- ggplot(df, aes(x=as.numeric(exceedances))) + 
  geom_histogram(aes(y=..density..), binwidth=0.04, size=0.4, colour="black", fill="lightgray", position="identity", center=0.02) +
  xlab(expression('z'[n])) + ylab("Density") + xlim(0,1) + ylim(0,10) +
  #geom_area(stat = "function", fun = dgpd, args = list(loc=0, scale=0.05567, shape=-0.01623), fill = "pink", alpha=0.5, xlim = c(0, 5)) +
  #geom_area(stat = "function", fun = dnsbeta, args = list(shape1=param$alpha, shape2=param$beta, min = 0, max = 9, log = FALSE), colour="black", fill = "dodgerblue2", alpha=0.2, xlim = c(0, 5)) +
  geom_area(stat = "function", fun = dgpd, args = list(mu=0, sigma=0.05567, xi=-0.01623), colour="black", fill = "orange", alpha=0.4, xlim = c(0, 5)) +
  theme_light() + 
...
p2 <- ggplot(df, aes(x=as.numeric(exceedances))) + 
  geom_histogram(aes(y=..density..), binwidth=0.04, size=0.4, colour="black", fill="lightgray", position="identity", center=0.02) +
  xlab(expression('z'[n])) + ylab("Density") + xlim(0,1) + ylim(0,10) +
  #geom_area(stat = "function", fun = dgpd, args = list(loc=0, scale=0.05567, shape=-0.01623), fill = "pink", alpha=0.5, xlim = c(0, 5)) +
...
p
setwd("C:/Users/rennien/OneDrive - Lancaster University/PhD/Simulation/Project 4 - Bike Visualisation/Images/PDF")
ggsave(p, filename = "gpd_fit.pdf",  device=cairo_pdf, bg = "transparent", height=3, width=5.5, unit="in")
